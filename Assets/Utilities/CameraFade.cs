...
public class CameraFade : MonoBehaviour
{
	...
	[SerializeField] private float m_FadeOutDuration = 2.0f;           // How long it takes to fade out seconds.
	[SerializeField] private float m_FadeInDuration = 2.0f; 	// How long it takes to fade out seconds.
...
//	}
	...
	{
		FadeOut(m_FadeOutDuration, fadeAudio);
		//Debug.Log("Fade out");
	...
	{
		FadeIn(m_FadeInDuration, fadeAudio);
	}
	...
	public IEnumerator BeginFadeOut (bool fadeAudio)
	{
		...
		if(m_FadedSnapshot && fadeAudio)
			m_FadedSnapshot.TransitionTo (m_FadeOutDuration);

		yield return StartCoroutine(BeginFade(m_FadeOutColor, m_FadeColor, m_FadeOutDuration));
	}
	...
	public IEnumerator BeginFadeIn (bool fadeAudio)
	{
		...
		if(m_DefaultSnapshot && fadeAudio)
			m_DefaultSnapshot.TransitionTo (m_FadeOutDuration);

		yield return StartCoroutine(BeginFade(m_FadeColor, m_FadeOutColor, m_FadeOutDuration));
	}
